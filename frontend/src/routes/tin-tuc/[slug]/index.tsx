...
export default component$(() => {
  ...
  const goToPage = (page: number) =>
    `/tin-tuc/?page=${page}${!_.isEmpty(currentCategory) ? "&type=" + currentCategory : ""}${
  ...
  const getCurrentBlogType = () => blogTypes.find((x) => x.attributes?.slug === currentCategory);
  return (
    <div class="w-100">
      <div class="hd-page">
                ...
                {blogs.map((x) => {
                  return (
                    <div key={x.id} class="news__item imp-grid-01__item">
                      <a
                        href={`/tin-tuc/${getCurrentBlogType()?.attributes?.slug}/${x.attributes?.slug}`}
