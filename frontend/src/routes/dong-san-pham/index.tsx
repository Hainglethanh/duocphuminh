...
import { ProductTypeContext, generateAxiosConfig, getImageUrl } from "~/utils/conts";

export const useGetProductList = routeLoader$(async ({ url }) => {
  try {
...
export const head: DocumentHead = () => {
  return {
    ...
    meta: [
      {
        name: "keywords",
        content: "Dược Phú Minh, sản phẩm Dược Phú Minh, Phú Minh, Dược phẩm Phú Minh, dòng sản phẩm Phú Minh",
      },
      {
        name: "description",
        content:
...
export default component$(() => {
  ...
  const goToPage = (page: number) =>
    `/dong-san-pham/?page=${page}${!_.isEmpty(currentCategory) ? "&type=" + currentCategory : ""}${
      !_.isEmpty(currentSearch) ? "&s=" + currentSearch : ""
    }`;

  const getCurrentProductType = () => productTypes.find((x) => x.attributes?.slug === currentCategory);
  return (
    <div class="w-100">
      <div class="hd-page">
        <div class="hd-page__wrap">
          <div class="hd-page__inner">
            <div class="hd-page__image">
              <img src="../uploads/4b6125ce-9f54-43cb-80d4-886d75b565d1.png" alt="" />
            </div>
            <div class="hd-page__content">
              <h1 class="hd-page__title text-uppercase font-02 color-white">S&#x1EA3;n ph&#x1EA9;m</h1>
              <form class="hd-navbar-form" method="get" action="/dong-san-pham">
                <button type="submit">
                  <i class="far fa-search"></i>
                </button>
                <input type="text" required name="s" placeholder="Nhập tên sản phẩm bạn muốn tìm" />
              </form>
            </div>
          </div>
        </div>
      ...
      <main>
        <div class="product-container">
          <div class="imp-container">
            <div id="dong-san-pham" class="product__header text-center pd-hd-01">
                ...
                {productTypes.map((x) => {
                  return (
                    <li key={x.id} class="product-cate active ">
                      <a href={`/dong-san-pham/?type=${x.attributes?.slug}`} class="product-cate__link">
                        ...
                        <h3 class="product-cate__name font-07 color-white">{x.attributes?.name}</h3>
                      </a>
                    </li>
                  );
                })}
            ...
            {currentSearch && (
              <div class="product__header text-center pd-hd-01" id="danh-sach-san-pham">
                <h3 class="font-02 color-02 text-uppercase">Kết quả tìm kiếm: {decodeURI(currentSearch)}</h3>
              </div>
            )}
            {currentCategory && (
              <div class="product__header text-center pd-hd-01" id="danh-sach-san-pham">
                <h3 class="font-02 color-02 text-uppercase">{getCurrentProductType()?.attributes?.name}</h3>
              </div>
            )}
            <div>
                  ...
                  {products?.map((x) => {
                    return (
                      <li key={x.id} class="product">
                        <a class="product__link" href={`/san-pham/${x.attributes?.slug}`}>
                          ...
                          <div class="product__content">
                            <h3 class="product__name font-05">{x.attributes?.name}</h3>
                            <div class="product__desc">{x.attributes?.introduce}</div>
                          </div>
                        </a>
                      </li>
                    );
